<?php
/**
* @version $Id: install_db.php 422 2012-04-30 16:01:05Z eska $
* @package phpmygpx
...
if(!$link) {
	...
	if($upgrade >= 7) {
		...
		if(!$column_exists) {
			...
			// fill these columns with defaults
			$query = "UPDATE IGNORE `${cfg['db_table_prefix']}waypoints` SET 
						`icon_file` = 'openlayers/img/marker-blue.png',
						`icon_size` = '22,25', 
						`icon_offset` = '-12,-25'; ";
			$result = mysql_query($query);
		}
		// fill existing icon related columns
		$query = "UPDATE IGNORE `${cfg['db_table_prefix']}waypoints` SET 
					`icon_file` = 'openlayers/img/marker-blue.png',
					`icon_size` = '22,25', 
					`icon_offset` = '-12,-25' 
