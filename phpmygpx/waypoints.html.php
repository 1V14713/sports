<?php
/**
* @version $Id: waypoints.html.php 428 2012-05-21 22:36:19Z eska $
* @package phpmygpx
...
class HTML_waypoints {
	...
	function editForm($url, $id, $wpt) {
		...
		// HTML layer for choosing waypoint icons, hidden by default
		echo "<div id='icon_layer' class='icon_layer' style='visibility:hidden; z-index:2000; position:absolute; top:130px; width:700px; background-color:#FFFFCC; border:5px solid #804000; padding:10px'>\n";
		echo "<div style='text-align:right;'><a href='javascript:hideMapIcons();'>";
		echo "<img src='images/cancel_12.png' title='"._CMN_CANCEL."'/></a></div>\n";
		
		echo "<table width='700px'>";
		foreach(scan_dir(MAP_ICONS_PATH, 0, 'FILETYPE_DIR') as $icon_dir) {
			// skip directories with names starting with '.' e.g. '.svn'
			if(strpos($icon_dir, '.') === 0)	continue;
			// print one table row for each map icon category
		...
		// HTML form for creating and editing waypoints
		echo "<form method='post' action='$url' class='filter'>\n";
		echo "<table width='100%'><tr><td><table>\n";
		echo "<tr><td><b>". _CMN_ICON .":</b> </td><td><input type='hidden' name='map_icon_url' value='".$wpt['icon_file']."' /><a href='javascript:showMapIcons();'><img id='map_icon' src='".$wpt['icon_file']."' border=0 /></a> (". _WPT_CLICK_TO_CHANGE .")</td></tr>\n";
		echo "<tr><td><b>". _CMN_DATE .":</b></td><td>". strftime(_DATE_FORMAT_LC2, 
			strtotime(($id)?($wpt['timestamp']):('now'))) ."</td></tr>\n";
		echo "<tr><td><b>". _CMN_LAT .":</b> </td><td><input type='text' name='wpt_lat' value='".$lat."' size=12 maxlength=12 onChange='updateMarkerPosition();' /></td></tr>\n";
		echo "<tr><td><b>". _CMN_LON .":</b> </td><td><input type='text' name='wpt_lon' value='".$lon."' size=12 maxlength=12 onChange='updateMarkerPosition();' /></td></tr>\n";
		...
		$map->embed();
		// hide marker on map if creating a new waypoint
		if(!$id) {
			echo "<script type='text/javascript'>\n";
			echo "	layerMarkers.setVisibility(false);\n";
			echo "</script>\n";
		}
		echo "<br />". _WPT_EDIT_MAP_MAN;
		echo "</td></tr></table>\n";
		echo "</form>\n";
	}
	...
	function deleteForm($url, $id) {
		...
		echo "<input type='submit' name='submit_btn' value='"._CMN_DELETE."' onClick='submit();'>\n";
