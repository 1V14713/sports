<?php
/**
* @version $Id: waypoints.php 426 2012-05-21 20:30:09Z eska $
* @package phpmygpx
...
define( 'DEFAULT_MAP_ICON', 'openlayers/img/marker-blue.png');
$DEBUG = FALSE;
...
function viewWaypoints($id, $page, $sort, $order, $limit) {
    ...
    if(mysql_num_rows($result)) {
		...
		while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			// use default icon for waypoint if none is set in db
			if($row['icon_file'] == '')
				$row['icon_file'] = DEFAULT_MAP_ICON;
...
function createWaypoints($id=0, $submit) {
	global $DEBUG, $cfg;
	HTML::heading(_MENU_WPT_CREATE, 3);
	if(continueEditWaypoints($id, $submit))
		HTML::message(_WPT_CREATED);
}
...
function continueEditWaypoints($id, $submit) {
	global $DEBUG, $cfg;
	if(!$cfg['public_host'] || check_password($cfg['admin_password'])) {
		if(!$submit) {
			...
			// set position offset for map icon
			if(strpos($icon_file, 'openlayers/img/marker') !== FALSE)
				$icon_offset = '-12,-25';
			else
				$icon_offset = '-10,-10';
