<?php
/**
* @version $Id: pois.php 436 2012-06-29 19:34:13Z eska $
* @package phpmygpx
...
function getWaypoints() {
    ...
    $query = "SELECT SQL_CALC_FOUND_ROWS * FROM `${cfg['db_table_prefix']}waypoints` ";
    /*
	if($option == "filter") {
	...
	*/
    ...
    if(mysql_num_rows($result)) {
		...
		// write table row for each waypoint
		while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			$description = "Altitude: ${row['altitude']} m<br />";
			$description .= "Lat: ".($row['latitude']/1000000)."<br />";
			$description .= "Lon: ".($row['longitude']/1000000)."<br />";
			$description .= "${row['timestamp']}<br />";
			$description .= "GPX-ID: <a href='traces.php?task=details&id=${row['gpx_id']}'>${row['gpx_id']}</a><br />";
			$description .= str_replace("\n", "<br>", $row['cmt']). "<br />";
			$description .= str_replace("\n", "<br>", $row['desc']) ."<br />";
			$description .= "<a href='waypoints.php?task=edit&id=${row['id']}'><img src='images/b_edit.png' /></a>";
			
			// write table row for datalayer
			echo ($row['latitude']/1000000)."\t".($row['longitude']/1000000).
				"\t${row['name']} \t${description}\t${row['icon_file']}".
	...
	*/
    ...
    if(mysql_num_rows($result)) {
		...
		while($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			echo "{\"width\":\"0\",\"time\":\"${row['timestamp']}\",\"lat\":\"".lat2y($row['latitude']/1000000)."\",\"photoid\":\"${row['id']}\",\"lon\":\"".lon2x($row['longitude']/1000000)."\",\"height\":\"0\",\"viewdir\":\"${row['image_dir']}\"}";
